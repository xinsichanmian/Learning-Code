...
the data set contains left-right hands motor imagery EEGNet signals form electrodes of C3, Cz, and C4
'''
...
# read raw EEGNet dataset of .gdf file
class RawEEGData:
    # rawData = list()
    # event = pd.DataFrame()
    # channel = list()
    ...
    # print event type information of EEGNet data set
    @staticmethod
    def print_type_info():
        print("EEGNet data set event information and index:")
        print("%12s\t%10s\t%30s" % ("Event Type", "Type Index", "Description"))
        print("%12d\t%10d\t%30s" % (276, 0, "Idling EEGNet (eyes open)"))
        print("%12d\t%10d\t%30s" % (277, 1, "Idling EEGNet (eyes closed"))
    ...
    for data_file in data_files:
        ...
        try:
            ...
            for event, event_data in trial_event.iterrows():
                trial_data = raw_eeg_data.rawData[:,
                             event_data['position']:event_data['position'] + event_data['duration']]
...
if __name__ == "__main__":
    # for test 1
    filename = "dataset/BCICIV_2b_gdf/B0302T.gdf"
    d = RawEEGData(filename)
    data = d.event[d.event['event index'] == 2]
    print(0, "\n", data.shape)

    # data_src = r"dataset/BCICIV_2b_gdf"
    # labels_src = r"dataset/true_labels2b"
